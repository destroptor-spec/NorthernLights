...
export const fetchArtistData = async (artistName) => {
    if (!artistName)
        return {};
        ...
        try {
            if (api === 'genius' && geniusApiKey) {
                ...
                if (res.ok) {
                    ...
                    const hit = hits.find((h) => h.type === 'song' && h.result.primary_artist.name.toLowerCase() === artistName.toLowerCase());
                ...
                if (res.ok) {
                    ...
                    if (json.artist) {
                        ...
                        if (!data.imageUrl && json.artist.image) {
                            ...
                            const largeImg = images.find((i) => i.size === 'mega' || i.size === 'extralarge' || i.size === 'large');
...
export const fetchAlbumImage = async (albumName, artistName) => {
    ...
    if (cached)
        return cached.imageUrl || undefined;
...
export const fetchGenreImage = async (genreName) => {
    if (!genreName)
        return undefined;
