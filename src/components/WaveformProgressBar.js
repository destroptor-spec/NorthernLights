...
function drawWaveform(canvas, peaks, progress, // 0-1
dpr, isDark) {
...
export const WaveformProgressBar = ({ audioUrl, currentTime, duration: rawDuration, onSeek, dbDuration, allowWaveformDecode = true, }) => {
    ...
    return (_jsxs("div", { className: "waveform-container", style: { position: 'relative', width: '100%', height: '48px', display: 'flex', alignItems: 'center' }, children: [_jsx("canvas", { ref: canvasRef, onClick: handleClick, onMouseMove: handleMouseMove, style: {
                    width: '100%',
                    height: '100%',
                    display: 'block',
                    cursor: 'pointer',
                    ...
                    pointerEvents: 'none',
                }, children: _jsx("span", { style: { fontSize: '0.65rem', color: isDark ? 'rgba(255,255,255,0.3)' : 'rgba(0,0,0,0.25)', letterSpacing: '0.1em' }, children: "Loading waveform\u2026" }) }))] }));
};
