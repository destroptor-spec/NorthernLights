...
export const AlbumDetail = () => {
    const { library, selectedItem, setPlaylist, navigateView, openContextMenu } = usePlayerStore();
    const [albumTitle, albumArtist] = (selectedItem || '').split('::::');
    // Filter tracks to just this album using the same two-pass heuristic as LibraryHome
    const albumTracks = useMemo(() => {
        const tracksForName = library.filter(t => t.album === albumTitle);
        const subAlbums = new Map();
        tracksForName.forEach(track => {
        ...
        for (const [explicitAA, subTracks] of subAlbums.entries()) {
            if (explicitAA !== '') {
                if (explicitAA === albumArtist)
                    return subTracks;
            }
            else {
                const uniqueArtists = new Set(subTracks.map(t => t.artist || 'Unknown Artist'));
                const derivedArtist = uniqueArtists.size === 1 ? Array.from(uniqueArtists)[0] : 'Various Artists';
                if (derivedArtist === albumArtist)
                    return subTracks;
            }
        }
        return [];
