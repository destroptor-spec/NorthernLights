...
export async function addTrack(track) {
    ...
    const sanitizeArray = (arr) => Array.isArray(arr) ? arr.map(sanitize) : arr;
    await db.query(`
...
export async function addDirectory(dirPath) {
    const db = await initDB();
...
export async function removeTracksByDirectory(dirPath) {
    const db = await initDB();
...
export async function recordPlayback(trackId) {
    const db = await initDB();
...
export async function recordSkip(trackId) {
    const db = await initDB();
...
const artistCache = new Map(); // name -> UUID
const albumCache = new Map(); // "title::::artist" -> UUID
const genreCache = new Map(); // name -> UUID
...
export async function getOrCreateArtist(name) {
    ...
    if (cached)
        return cached;
    const db = await initDB();
    const res = await db.query(`INSERT INTO artists (name) VALUES ($1) ON CONFLICT (name) DO UPDATE SET name = EXCLUDED.name RETURNING id`, [name]);
    const id = res.rows[0].id;
    artistCache.set(name, id);
    return id;
}
export async function getOrCreateAlbum(title, artistName) {
    const key = `${title}::::${artistName || ''}`;
    const cached = albumCache.get(key);
    if (cached)
        return cached;
    const db = await initDB();
    const res = await db.query(`INSERT INTO albums (title, artist_name) VALUES ($1, $2) ON CONFLICT (title, artist_name) DO UPDATE SET title = EXCLUDED.title RETURNING id`, [title, artistName || null]);
    const id = res.rows[0].id;
...
export async function getOrCreateGenre(name) {
    ...
    if (cached)
        return cached;
    const db = await initDB();
    const res = await db.query(`INSERT INTO genres (name) VALUES ($1) ON CONFLICT (name) DO UPDATE SET name = EXCLUDED.name RETURNING id`, [name]);
    const id = res.rows[0].id;
...
export async function migrateEntityIds() {
    const db = await initDB();
    const res = await db.query('SELECT id, artist, albumartist, artists, album, genre FROM tracks WHERE artist_id IS NULL OR album_id IS NULL OR genre_id IS NULL');
    ...
    for (const row of res.rows) {
        ...
        const albumArtistName = row.albumartist || row.artist;
        ...
        try {
            ...
            if (rawArtists) {
                if (typeof rawArtists === 'string') {
                    try {
                        individualArtists = JSON.parse(rawArtists);
                    }
                    catch { }
                }
                ...
                const parts = row.artist.split(/\s+(?:feat\.?|ft\.?|featuring|&)\s+(?!$)/i).map((s) => s.trim()).filter(Boolean);
                if (parts.length > 0)
                    individualArtists = parts;
            }
        ...
        try {
            if (albumTitle)
                albumId = await getOrCreateAlbum(albumTitle, albumArtistName || null);
        }
        catch { }
        try {
            if (genreName)
        ...
        await db.query('UPDATE tracks SET artist_id = COALESCE($1, artist_id), album_id = COALESCE($2, album_id), genre_id = COALESCE($3, genre_id) WHERE id = $4', [artistId, albumId, genreId, trackId]);
        count++;
    }
...
// PLAYLISTS API 
// ==========================================
export async function createPlaylist(id, title, description = null, isLlmGenerated = false) {
    const db = await initDB();
    await db.query(`
...
export async function deleteOldLlmPlaylists(maxAgeMs) {
    const db = await initDB();
...
export async function getGenreMatrixCache() {
    ...
    if (res.rows.length === 0 || !res.rows[0].matrix)
        return {};
    const matrix = res.rows[0].matrix;
    try {
...
export async function updateGenreMatrixCache(matrix) {
    const db = await initDB();
...
export async function updateMacroMatrix(matrix) {
    const db = await initDB();
    const matrixStr = JSON.stringify(matrix);
    await db.query(`
...
export async function upsertSubGenreMapping(subGenre, macroGenre) {
    const db = await initDB();
...
export async function getSubGenreMappings() {
    const db = await initDB();
...
export async function getMacroGenreFromKNN(vector) {
    const db = await initDB();
